...
export default function ResumeClientPage() {
  ...
  const [data, setData] = useState<Awaited<ReturnType<typeof getPublicResumeData>> | null>(null);

  useEffect(() => {
    void getPublicResumeData(uid).then(setData);
  }, [uid]);
    ...
    <div className="mx-auto max-w-4xl space-y-6 py-8">
      <div className="rounded-2xl border p-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold">{String(profile.displayName ?? "Kinet Athlete")}</h1>
            <p className="text-muted-foreground">
              {[String(role.sport ?? ""), String(role.position ?? ""), String(role.team ?? "")].filter(Boolean).join(" · ")}
            </p>
            <p className="mt-2 text-sm text-muted-foreground">@{String(profile.username ?? uid.slice(0, 8))}</p>
          </div>
          <Link href={`/media-kit/${uid}`} className="rounded-full border px-4 py-2 text-sm hover:bg-muted/40">Open Media Kit</Link>
        </div>
