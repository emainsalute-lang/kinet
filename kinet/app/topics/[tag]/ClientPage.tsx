...
export default function TopicClientPage() {
  ...
  useEffect(() => subscribeToTopicPosts(tag, setPosts), [tag]);

  useEffect(() => {
    void getCurrentUserSettings().then((settings) =>
      setIsFollowing(settings.followedTopics.includes(tag.toLowerCase()))
    );
  }, [tag]);
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Button
              ...
              onClick={() =>
                void toggleTopicFollow(tag, isFollowing).then(() => setIsFollowing((current) => !current))
              }
            >
