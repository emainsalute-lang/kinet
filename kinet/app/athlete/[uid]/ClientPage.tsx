...
export default function AthleteLandingPageContent() {
  ...
  useEffect(() => {
    if (!uid) return;
    void getSeoProfilePageData(uid).then((next) => setData(next));
    return subscribeToUserPosts(uid, setPosts);
  }, [uid]);
  ...
  useEffect(() => {
    ...
    void import("@/lib/phase6").then(async (mod) => {
      ...
      setFundraisers(all.filter((item: FundraiserRecord) => item.ownerRefId === uid || item.ownerId === uid));
    });
  }, [uid]);
    ...
    <div className="mx-auto max-w-5xl py-10">
      <section className="rounded-3xl border bg-gradient-to-r from-primary/10 to-secondary/10 p-8">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href={`/profile/${uid}`} className="rounded-xl bg-primary px-5 py-3 font-semibold text-primary-foreground">Open Profile</Link>
          <Link href={`/newsletter/${uid}`} className="rounded-xl border px-5 py-3 font-semibold">Newsletter</Link>
          <Link href={`/blog/${uid}`} className="rounded-xl border px-5 py-3 font-semibold">Blog</Link>
        </div>
      </section>
