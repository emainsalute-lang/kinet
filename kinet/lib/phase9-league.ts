...
export async function getLeagueHubSnapshot(): Promise<LeagueHubSnapshot> {
  const [
    ...
    getCollection("phase9Protests", (id, data) => ({ id, teamName: String(data.teamName ?? ""), gameLabel: String(data.gameLabel ?? ""), summary: String(data.summary ?? ""), status: (data.status === "reviewing" || data.status === "resolved" ? data.status : "submitted") as "submitted" | "reviewing" | "resolved", createdAt: mapTimestamp(data, "createdAt") })),
    ...
    getCollection("phase9FreeAgents", (id, data) => ({ id, athleteName: String(data.athleteName ?? ""), sport: String(data.sport ?? ""), position: String(data.position ?? ""), status: (data.status === "contacted" || data.status === "signed" ? data.status : "available") as "available" | "contacted" | "signed", createdAt: mapTimestamp(data, "createdAt") })),
