...
async function createRecord(name: string, data: Record<string, unknown>) {
  ...
  await addDoc(collection(db!, name), {
    ownerId: user.uid,
    ownerName: user.displayName || user.email || "Kinet User",
    ...data,
...
export async function getTravelLegacySnapshot(): Promise<TravelLegacySnapshot> {
  const [
    ...
    getCollection("phase8Reimbursements", (id, data) => ({ id, teamId: String(data.teamId ?? ""), requesterName: String(data.requesterName ?? ""), expenseLabel: String(data.expenseLabel ?? ""), amountLabel: String(data.amountLabel ?? ""), status: (data.status === "approved" || data.status === "paid" ? data.status : "pending") as "pending" | "approved" | "paid", createdAt: mapTimestamp(data, "createdAt") })),
    ...
    getCollection("phase8AcademyRegistrations", (id, data) => ({ id, academyName: String(data.academyName ?? ""), athleteName: String(data.athleteName ?? ""), ageGroup: String(data.ageGroup ?? ""), status: (data.status === "accepted" || data.status === "waitlist" ? data.status : "pending") as "pending" | "accepted" | "waitlist", createdAt: mapTimestamp(data, "createdAt") })),
